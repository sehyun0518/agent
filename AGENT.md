...
> 컨텍스트 윈도우는 작업 공간이지 데이터베이스가 아니다.

아래 두 개의 "여기 없다"도, 브리프와 기반을 가른 것도, `.claude`·`.codex`를 생성물로
둔 것도 이 한 문장에서 나온다. 기억해야 할 것을 컨텍스트에 두면 세션과 함께 사라진다.

지식은 이 문서와 프로파일이 파일로 지킨다. 실행 중에 생긴 **증거**는 `artifact` 경로가
지킨다 — 검증기가 코어 증거마다 경로 선언을 요구한다. 다만 그 경로에 실제로 파일을 쓰는
주체는 아직 없다 (`docs/operations.md` §0).
...
- 코어 증거는 `artifact` 경로 없이 선언할 수 없다. 경로가 필요 없으면 그것은 증거가 아니라 신호다.
- 동작 구현은 같은 계층의 `test.<layer>.red-confirmed` 없이 시작할 수 없다. 단 계약 스캐폴드(`logic-scaffold`·`ui-scaffold`)는 동작을 담지 않으므로 red 앞에 온다.

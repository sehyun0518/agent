...
`workflow`는 계층의 층이 아니라 **실행의 차원**이다. `workflow.started` 같은 이벤트는
없고, 어느 흐름을 따르는 실행인지를 이벤트가 들고 다닌다.

단계 집합으로는 추론되지 않는다 — `bugfix`의 단계가 전부 `change`에 들어 있어 중간에
멈춘 `change`가 완주한 `bugfix`와 같은 집합을 남긴다. 그리고 **중간에 멈춘 실행이야말로
보고 싶은 것**이다 (ADR-0030).

"이 시스템이 **이 종류의 일**을 얼마나 신뢰성 있게 끝내나"의 종류가 이것이다.
...
**집계를 하네스 안에서도 만들지 않는다.** Adapter가 넘긴 관측 시스템이 한다 — 그것이
그 시스템이 하는 일이고, 하네스가 다시 만들면 두 곳에 같은 계산이 생긴다. 하네스가 지는
책임은 **무엇으로 갈라 볼 수 있는지**, 즉 차원을 내보내는 것까지다 (ADR-0031).

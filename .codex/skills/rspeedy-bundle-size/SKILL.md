...
metadata:
  ...
  source: https://github.com/lynx-community/skills/tree/release/skills/rspeedy-bundle-size
  version: "1.0.0"
---

> **📚 Lynx 레퍼런스.** rspeedy/Lynx 앱의 번들 크기를 분석하고 줄입니다.
> `vanilla-lynx`가 번들 작업을 이 스킬로 라우팅합니다. 규칙 팩이 아니라 조회 자료라
...
`skills/rspeedy-bundle-size`(브랜치 `release`)를 Apache-2.0으로 벤더링했습니다. 본문은 원본
그대로입니다. 이 저장소가 더한 것은 frontmatter의 `license`·`metadata`와 상단 안내,
그리고 이 절뿐입니다.

`references/assets/` 7개를 함께 가져왔습니다. `SKILL.md`가 `scan-levers.mjs`를
직접 실행하고 나머지는 사용자 프로젝트에 꽂는 템플릿이라, 빼면 문서가 없는 파일을
가리키게 됩니다.

...
플랫폼이 그것을 집행한다(ADR-0015). **정책 전부를 덮는다는 뜻은 아니다** —
`destructive-approval`의 경우 플랫폼은 사람을 멈춰 세우는 것까지 하고, 승인을
`approval-record`로 남기는 것과 거부를 재시도로 우회하지 못하게 하는 것은 여전히
훅 런타임을 기다린다. 무엇이 남았는지는 레지스트리의 `pending`이 갖는다. **강제 수준은 플랫폼마다 다를 수 있다.** 어느

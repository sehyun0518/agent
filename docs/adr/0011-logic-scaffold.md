...
> **2026-09-01 재검토 — 뒤집었다. `bugfix`에도 넣는다.**
>
> 위 논거는 "버그가 있는 모듈"만 봤고 **수정이 도입하는 모듈**을 보지 않았다. 날짜
> 정규화 함수를 뽑아내 고치는 식의 수정은 흔하고, 그 헬퍼를 재현 테스트가 import하는
> 순간 같은 교착에 빠진다. 재현할 버그가 기존 코드에 있다는 것과 수정에 새 파일이
> 필요하다는 것은 다른 명제다.
>
> `ui-scaffold`가 이미 `bugfix`에 있는 것과도 어긋났다. UI 수정에는 새 컴포넌트가
> 필요할 수 있다고 인정하면서 로직 수정에는 새 모듈이 필요 없다고 본 근거가 없다.
>
> 대가는 단계 하나다. 대부분의 수정에서 "만들 것 없음"으로 끝난다 — `change`에서와 같다.
>
> 이 뒤집기로 규칙에 예외가 사라졌다. **red 게이트가 걸린 구현 변형에 스캐폴드 변형이
> 선언돼 있으면, 그것을 쓰는 단계가 반드시 앞에 있다.** 검증기가 그것을 지킨다
> (`tooling/validators/workflow-scaffold.mjs`).
...
- ~~`bugfix`에서 신규 모듈이 필요한 사례가 실제로 나오면.~~ 발동했다 — 위 재검토 참고.
